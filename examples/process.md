...
there. In this example there are three input possibilities. The first one should be possible in every organization.
...
would be "high" in this scenario the incident is severe as systems or data is affected which might harm the company.
Here an immediate reaction is necessary to fight against this threat. The last level is "critical" here the company is
in severe danger, and the highest level needs to perform actions to combat this incident.
...
triage phase the event is analyzed. This is needed to get an overview of the damage and the ongoing threat to the 
company's systems.  
...
When the incident is analyzed and the threats are identified the incident can be contained. In this phase the incident
...
the organization should be informed about this incident to further harden the whole environment.  
